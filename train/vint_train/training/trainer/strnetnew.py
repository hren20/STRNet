...
class STRNetNewTrainer(BaseTrainer):
    ...
    def _create_model(self) -> torch.nn.Module:
        ...
        # 构建STRNet视觉编码器
        vision_encoder = STRNetNew_Extractor(
            context_size=model_config["context_size"],
            obs_encoder=model_config.get("obs_encoder", "efficientnet-b0"),
            obs_encoding_size=model_config["encoding_size"],
            gnn_layers=model_config.get("gnn_layers", 3),
            K_list=model_config.get("K_list", [8,4,2,1])
        )
        ...
        dist_pred_net = DenseNetwork(
            embedding_dim=model_config["encoding_size"]
        )
    ...
    def _train_step(self, batch: tuple, epoch, batch_idx: int) -> Dict[str, torch.Tensor]:
        ...
        # 前向传播
        obs_cond = self.model.vision_encoder(obs_tensor, goal_tensor)
        noise, noise_pred = self._forward_diffusion(naction, obs_cond)
        dist_pred = self.model.dist_pred_net(obs_cond)
        # 损失计算
        losses = self._compute_train_losses(noise_pred, dist_pred, noise, dist_label, action_mask)
        ...
        # 如果属性不存在，则初始化集合
        if not hasattr(self, 'train_visualized_epochs'):
            self.train_visualized_epochs = set()

        if self._should_visualize(batch_idx, self.config["image_log_freq"]) and epoch not in self.train_visualized_epochs:
            # 解包可视化数据
            viz_obs, viz_goal, viz_goal_pos = viz_data  # 新增解包语句
            self._visualize_diffusion_actions(
                batch_obs_images=obs_tensor,
            ...
            self.train_visualized_epochs.add(epoch)
    ...
    def _compute_train_losses(self, noise_pred, dist_pred, naction, dist_label, action_mask):
        """训练阶段损失计算"""
        # 扩散损失（带掩码）
    ...
    def _eval_step(self, batch: tuple, epoch: int, batch_idx: int, eval_type: str) -> Dict[str, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            # 如果属性不存在，则初始化集合
            if not hasattr(self, 'eval_visualized_epochs'):
                self.eval_visualized_epochs = set()

            if self._should_visualize(batch_idx, self.config["image_log_freq"]) and epoch not in self.eval_visualized_epochs:
                # 解包可视化数据
                viz_obs, viz_goal, viz_goal_pos = viz_data  # 新增解包语句
                self._visualize_diffusion_actions(
                    batch_obs_images=obs_tensor,
                ...
                self.eval_visualized_epochs.add(epoch)
    ...
    def _denoise_loop(self, model, obs, goal: torch.Tensor, num_samples) -> torch.Tensor:
        """逆向扩散核心逻辑"""
    ...
    def _visualize_diffusion_actions(
        ...
        pred_dict = self._generate_visualization_predictions(chunk, self.num_samples)
    ...
    def _plot_trajectory(self, ax: plt.Axes, traj_data: np.ndarray, goal_pos: np.ndarray):
        """核心轨迹绘制方法"""
        # 动态颜色配置
        num_samples = self.num_samples  # 最后一个是GT
        colors = (
